...
int VpnToPhyPage(int vpn)
{
  ...
  //you can refer to PageOutPageIn(int vpn) to see how an entry was created in ipt
	...
	// If not in TLB, then check page table
	for (int i = 0; i < NumPhysPages; i++) {
...
void InsertToTLB(int vpn, int phyPage)
{
  ...
  //your code to find an empty in TLB or to replace the oldest entry if TLB is full
	...
	// If TLB has empty entries, that means the TLB if being populate the first time
	for (i = 0; i < TLBSize; i++) {
	...
	// If no empty page in tlb, then find the oldest one, which is at FIFOPointer
	if (i == TLBSize) {
		i = FIFOPointer;
...
int lruAlgorithm(void)
{
  ...
  int phyPage = 0;
	...
	int leastRecentlyUsedTick = 2147483647;
	
	// If empty page, then return that
	// Else find time with the smollest tick (referenced long time ago)
	for (int i = 0; i < NumPhysPages; i++) {
		if (!(memoryTable[i].valid)) {
			phyPage = i;
			break;
		}
		if (memoryTable[i].lastUsed < leastRecentlyUsedTick) {
			leastRecentlyUsedTick = memoryTable[i].lastUsed;
			phyPage = i;
		}	
	}
